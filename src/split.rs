...
pub fn split_file(file_path: &str, split_size_mib: u64, output_path: &str) -> Result<()> {
    let metadata = std::fs::metadata(file_path)
        .with_context(|| format!("Cannot stat file: {}", file_path))?;
    let file_size = metadata.len();
    
    // 1 MiB = 1024 * 1024 Bytes
    let part_size = split_size_mib * 1024 * 1024; 
    ...
    println!(
        "Splitting {} into {} parts of {} MiB ({} bytes) each",
        file_path, num_parts, split_size_mib, part_size
    );
    ...
    for i in 1..=num_parts {
        // Neues Namensschema: z.B. datei.part001.endung
        let part_name = format!("{}.part{:03}{}", name_without_ext, i, ext);
